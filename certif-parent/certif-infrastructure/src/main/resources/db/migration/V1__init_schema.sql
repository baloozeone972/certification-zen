...
CREATE
EXTENSION IF NOT EXISTS "pg_trgm"; -- Recherche full-text trigram
...
COMMENT
ON TABLE  users                          IS 'Comptes utilisateurs CertifApp';
COMMENT
ON COLUMN users.password_hash            IS 'BCrypt hash force 12 minimum — plaintext INTERDIT';
COMMENT
ON COLUMN users.role                     IS 'USER=standard, ADMIN=back-office, PARTNER=accès API';
COMMENT
ON COLUMN users.subscription_tier        IS 'FREE=freemium, PRO=illimité, PACK=certification unique';
COMMENT
ON COLUMN users.locale                   IS 'BCP-47 : fr, en, ar — pour i18n emails et UI';
COMMENT
ON COLUMN users.timezone                 IS 'IANA timezone : Europe/Paris, Africa/Casablanca, America/Montreal';
COMMENT
ON COLUMN users.stripe_customer_id       IS 'ID Stripe — NULL pour les utilisateurs FREE sans CB';
...
COMMENT
ON COLUMN user_preferences.theme                    IS 'light, dark ou system (suit les préfs OS)';
COMMENT
ON COLUMN user_preferences.last_certification_id    IS 'Dernière certification consultée — reprise rapide';
COMMENT
ON COLUMN user_preferences.free_mode_duration_min   IS '0 = illimité';
...
COMMENT
ON COLUMN certifications.id                  IS 'Slug interne : java21, aws_ccp, ocp21';
COMMENT
ON COLUMN certifications.code                IS 'Code officiel : 1Z0-830, CLF-C02, SAA-C03';
COMMENT
ON COLUMN certifications.exam_question_count IS 'Nombre de questions tirées par examen';
COMMENT
ON COLUMN certifications.passing_score       IS 'Seuil en % pour valider (ex: 68 pour OCP)';
COMMENT
ON COLUMN certifications.exam_type           IS 'MCQ=QCM, PRACTICAL=pratique lab (CKA/CKAD), MIXED';
...
COMMENT
ON COLUMN certification_themes.code          IS 'Slug URL : lambda_streams, virtual_threads';
COMMENT
ON COLUMN certification_themes.label         IS 'Nom affiché issu de config.json';
COMMENT
ON COLUMN certification_themes.weight_percent IS 'Pondération officielle — NULL si non publiée';
...
COMMENT
ON COLUMN questions.legacy_id            IS 'ID original JSON : JAVA-001, AWS-CCP-065 — traçabilité migration';
COMMENT
ON COLUMN questions.statement            IS 'Énoncé — peut contenir du Markdown pour les blocs de code';
COMMENT
ON COLUMN questions.explanation_original IS 'Texte original importé depuis CertiPrep Engine — référence immuable';
COMMENT
ON COLUMN questions.explanation_enriched IS 'Version enrichie par IA ou manuellement — NULL si non enrichie';
COMMENT
ON COLUMN questions.explanation_status   IS 'ORIGINAL=non relu, AI_DRAFT=généré, HUMAN_VALIDATED=approuvé';
COMMENT
ON COLUMN questions.ai_confidence_score  IS 'Score certitude IA 0.0-1.0 — guide la relecture humaine';
COMMENT
ON COLUMN questions.official_doc_url     IS 'Lien doc officielle Oracle/AWS/CNCF — pour l enrichissement';
...
COMMENT
ON COLUMN question_options.label         IS 'Lettre A/B/C/D/E affichée';
COMMENT
ON COLUMN question_options.is_correct    IS 'TRUE = bonne réponse — plusieurs TRUE possibles pour MULTIPLE_CHOICE';
COMMENT
ON COLUMN question_options.display_order IS 'Ordre original depuis options[] JSON (0-based → A/B/C/D)';
...
COMMENT
ON COLUMN exam_sessions.duration_seconds IS 'Durée RÉELLE passée = ended_at - started_at en secondes';
COMMENT
ON COLUMN exam_sessions.correct_count    IS 'Nombre de bonnes réponses — dénormalisé pour les performances';
COMMENT
ON COLUMN exam_sessions.percentage       IS 'correct_count*100/total_questions — CHECK 0-100';
COMMENT
ON COLUMN exam_sessions.passed           IS 'TRUE si percentage >= certifications.passing_score';
...
COMMENT
ON COLUMN user_answers.selected_option  IS 'NULL = question ignorée (remplace selected_answer=-1 de SQLite)';
COMMENT
ON COLUMN user_answers.is_skipped       IS 'Flag explicite pour questions non répondues';
COMMENT
ON COLUMN user_answers.response_time_ms IS 'Temps entre affichage question et sélection réponse en ms';
...
COMMENT
ON COLUMN courses.content_markdown IS 'Source Markdown — éditable par les admins';
COMMENT
ON COLUMN courses.content_html     IS 'HTML pré-rendu pour performance côté client';
COMMENT
ON COLUMN courses.ai_status        IS 'Statut de validation du contenu généré par IA';
...
COMMENT
ON COLUMN flashcards.question_id  IS 'Source : question du corpus — NULL si générée depuis cours';
COMMENT
ON COLUMN flashcards.course_id    IS 'Source : fiche de cours — NULL si générée depuis question';
COMMENT
ON COLUMN flashcards.ai_generated IS 'TRUE = générée par LangChain4j, FALSE = saisie manuelle';
...
COMMENT
ON COLUMN user_flashcard_progress.ease_factor    IS 'Facteur de facilité SM-2, minimum 1.3';
COMMENT
ON COLUMN user_flashcard_progress.interval_days  IS 'Intervalle en jours avant prochaine révision';
COMMENT
ON COLUMN user_flashcard_progress.next_review_date IS 'Date planifiée pour la prochaine révision';
...
COMMENT
ON TABLE  user_sm2_schedule IS 'Planification répétition espacée SM-2 par question et utilisateur';
COMMENT
ON COLUMN user_sm2_schedule.due_date IS 'Date à partir de laquelle la question doit être révisée';
...
COMMENT
ON COLUMN user_diagnostic_results.score_by_domain            IS 'JSONB : {domain: score} ex: {"Lambda":75, "POO":60}';
COMMENT
ON COLUMN user_diagnostic_results.skill_map                   IS 'JSONB : {skill: confidence_0_to_1}';
COMMENT
ON COLUMN user_diagnostic_results.recommended_certifications  IS 'JSONB : [{id, rationale, priority}]';
...
COMMENT
ON COLUMN user_certification_paths.generated_path IS 'JSONB : [{certificationId, order, rationale, estimatedWeeks}]';
COMMENT
ON COLUMN user_certification_paths.role_type      IS 'Ex: "Java Developer", "DevOps Engineer", "Cloud Architect"';
...
COMMENT
ON COLUMN weekly_coach_reports.study_plan IS 'JSONB : planning de révision de la semaine par jour et thème';
COMMENT
ON COLUMN weekly_coach_reports.sent_at    IS 'NULL si non encore envoyé';
COMMENT
ON COLUMN weekly_coach_reports.opened_at  IS 'NULL si email non ouvert (tracking pixel)';
...
COMMENT
ON COLUMN job_market_data.country_code      IS 'ISO 3166-1 alpha-2 : FR, MA, TN, BE, CH, CA, CI, SN';
COMMENT
ON COLUMN job_market_data.median_salary_eur IS 'Salaire médian en EUR (normalisé par taux de change)';
COMMENT
ON COLUMN job_market_data.top_companies     IS 'JSONB : [{name, jobCount, logoUrl}]';
COMMENT
ON COLUMN job_market_data.trend_data        IS 'JSONB : {monthly: [{month, count}], growth12m: 0.15}';
COMMENT
ON COLUMN job_market_data.expires_at        IS 'Cache TTL : 7 jours par défaut';
...
COMMENT
ON COLUMN weekly_challenges.question_ids      IS 'Array UUIDs des questions — 10 questions recommandées';
COMMENT
ON COLUMN weekly_challenges.total_participants IS 'Dénormalisé pour le classement — mis à jour par trigger';
...
COMMENT
ON COLUMN challenge_participations.badge_earned IS 'Ex: GOLD, SILVER, BRONZE, TOP_10_PERCENT';
COMMENT
ON COLUMN challenge_participations.rank         IS 'NULL jusqu à la clôture du challenge';
...
COMMENT
ON COLUMN study_groups.invite_code IS 'Code unique de 8 caractères pour rejoindre le groupe';
...
COMMENT
ON COLUMN certified_wall_profiles.prep_duration_weeks IS 'Durée de préparation auto-déclarée en semaines';
...
COMMENT
ON COLUMN notifications.type IS 'CHALLENGE_REMINDER, COACH_REPORT, BADGE_EARNED, REVIEW_DUE, etc.';
...
COMMENT
ON COLUMN push_subscriptions.p256dh IS 'Clé publique WebPush encodée base64url';
COMMENT
ON COLUMN push_subscriptions.auth   IS 'Secret d authentification WebPush encodé base64url';
