...
DECLARE
    ...
    v_theme_id   UUID;
    v_q1_id      UUID := uuid_generate_v4();
    v_q2_id      UUID := uuid_generate_v4();
    v_q3_id      UUID := uuid_generate_v4();
    v_q4_id      UUID := uuid_generate_v4();
    v_q5_id      UUID := uuid_generate_v4();
BEGIN
    -- Récupération du theme_id pour 'virtual_threads'
    SELECT id INTO v_theme_id
    FROM certification_themes
    WHERE certification_id = v_cert_id AND code = 'virtual_threads'
    LIMIT 1;
    ...
    INSERT INTO question_options (id, question_id, label, text, is_correct, display_order) VALUES
        (uuid_generate_v4(), v_q1_id, 'A', 'Thread.ofVirtual().start(() -> {})', TRUE,  0),
    ...
    INSERT INTO question_options (id, question_id, label, text, is_correct, display_order) VALUES
        (uuid_generate_v4(), v_q2_id, 'A', 'Executors.newVirtualThreadPerTaskExecutor()', TRUE,  0),
    ...
    SELECT id INTO v_theme_id
    FROM certification_themes
    WHERE certification_id = v_cert_id AND code = 'pattern_matching'
    LIMIT 1;
    ...
    INSERT INTO question_options (id, question_id, label, text, is_correct, display_order) VALUES
        (uuid_generate_v4(), v_q3_id, 'A', '"positif"', TRUE,  0),
    ...
    SELECT id INTO v_theme_id
    FROM certification_themes
    WHERE certification_id = v_cert_id AND code = 'records_sealed'
    LIMIT 1;
    ...
    INSERT INTO question_options (id, question_id, label, text, is_correct, display_order) VALUES
        (uuid_generate_v4(), v_q4_id, 'A', 'Le constructeur compact', FALSE, 0),
    ...
    SELECT id INTO v_theme_id
    FROM certification_themes
    WHERE certification_id = v_cert_id AND code = 'structured_concurrency'
    LIMIT 1;
    ...
    INSERT INTO question_options (id, question_id, label, text, is_correct, display_order) VALUES
        (uuid_generate_v4(), v_q5_id, 'A', 'StructuredTaskScope', TRUE,  0),
    ...
    UPDATE certifications SET total_questions = 5 WHERE id = v_cert_id;
