...
DECLARE
    ...
    v_theme_id    UUID;
    v_option_text TEXT;
    v_labels      CHAR(1)[] := ARRAY['A','B','C','D','E'];
    v_i           INT;
BEGIN
    -- Résolution du thème
    ...
    IF EXISTS (SELECT 1 FROM questions WHERE legacy_id = p_legacy_id) THEN
        RETURN (SELECT id FROM questions WHERE legacy_id = p_legacy_id);
    END IF;

    -- Insertion de la question
    INSERT INTO questions (
        legacy_id, certification_id, theme_id, statement,
        difficulty, question_type, explanation_original
    ) VALUES (
        p_legacy_id, p_certification_id, v_theme_id, p_statement,
        p_difficulty, 'SINGLE_CHOICE', p_explanation
...
COMMENT ON FUNCTION import_question IS
...
COMMENT ON VIEW v_import_progress IS
